...
# =================================================================
# CREATE STRIPE PAYMENT LINKS FOR BILLED PERIOD
# =================================================================
...
def stripe_create_payment_links_for_billed_period(docname):
    ...
    print("\n" + "=" * 100)
    print("STARTING STRIPE PAYMENT LINK CREATION")
    print("=" * 100)
    ...
    while True:
        ...
        #
        # IMPORTANT:
        # This DB query is executed in MAIN Frappe thread.
        # ---------------------------------------------------------
        ...
        invoices = frappe.get_all(
            ...
            filters={
                ...
                # Draft invoices
                # Change to 1 if you want submitted invoices
                "docstatus": 0,
        ...
        # =========================================================
        # FETCH ALL CUSTOMER EMAILS BEFORE THREADING
        #
        # IMPORTANT:
        # No Frappe DB access inside worker threads.
        # =========================================================
        ...
        for index, invoice in enumerate(
            ...
            try:
                ...
                # -------------------------------------------------
                # STORE EVERYTHING REQUIRED BY WORKER
                # -------------------------------------------------
    ...
    print(
        "PAYMENT LINK CREATION COMPLETED"
    )
...
def create_single_payment_link(
    ...
    for attempt in range(
        ...
        try:
            ...
            # -----------------------------------------------------
            # OPTIONAL:
            # If you want Stripe to collect customer email,
            # uncomment this:
            #
            # "customer_creation":
            #     "always"
            #
            # Note:
            # Payment Links do not use Checkout Session's
            # customer_email parameter in the same way.
            # -----------------------------------------------------

            print(
                f"[{invoice.name}] "
                f"Stripe Payment Link payload prepared."
            )
            ...
            response = requests.post(
                ...
                headers={
                    ...
                    # IDEMPOTENCY KEY
                    #
                    # Same invoice = same key.
                    #
                    # If Stripe receives the request but ERPNext
                    # doesn't receive the response, retrying will
                    # not create another Payment Link.
                    # -------------------------------------------------

                    "Idempotency-Key":
                        (
                            "erpnext-payment-link-"
                            f"{invoice.name}"
                        )
                },
            ...
            if not payment_link:
                ...
                return {
                    ...
                    "error":
                        (
                            "Payment link URL missing "
                            "from Stripe response. "
                            f"Payment Link ID: "
                            f"{payment_link_id}"
                        )
                }
...
@frappe.whitelist(allow_guest=True)
def new_without_payment_stripe_webhook():
    ...
    try:
        ...
        for row in invoice_rows:
            ...
            company = invoice.company
            greatgrandparent_name=invoice.greatgrandparent_name
            grandparent_name=invoice.grandparent_name
...
def new_payment_stripe_webhook():
    ...
    try:
        ...
        # -----------------------------------------
        # Get Stripe Payment Link ID
        # -----------------------------------------

        stripe_payment_link_id = (
            payment_collection.stripe_payment_link_id
        )
        ...
        for row in invoice_rows:
            ...
            greatgrandparent_name = (
                invoice.greatgrandparent_name
            )

            grandparent_name = (
                invoice.grandparent_name
            )
        ...
        stripe_payment_link_deactivated = False
        ...
        if stripe_payment_link_id:
            ...
            try:
                ...
                response = requests.post(
                    ...
                    f"https://api.stripe.com/v1/payment_links/"
                    f"{stripe_payment_link_id}",
                ...
                if response.status_code != 200:
                    ...
                    frappe.log_error(
                        response.text,
                        "Stripe Payment Link Deactivation Failed"
                    )

                else:

                    stripe_result = response.json()

                    if stripe_result.get("active") is False:

                        stripe_payment_link_deactivated = True
                        ...
                        frappe.log_error(
                            str(stripe_result),
                            "Stripe Payment Link "
                            "Deactivation Unexpected Response"
                        )
                ...
                frappe.log_error(
                    frappe.get_traceback(),
                    "Stripe Payment Link Deactivation Error"
                )
        ...
        if stripe_payment_link_deactivated:

            # -----------------------------------------
            # Optional: Mark Link Inactive
            # -----------------------------------------
            ...
            if frappe.get_meta(
                ...
                payment_collection.db_set(
                    "stripe_payment_link_active",
                    0
                )

            # -----------------------------------------
            # Optional: Mark Payment Status Paid
            # -----------------------------------------
        ...
        return {
            ...
            "stripe_payment_link_deactivated":
                stripe_payment_link_deactivated,
